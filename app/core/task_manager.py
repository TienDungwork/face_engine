...
class TaskManager:
    ...
    async def start_scheduled_tasks(self):
        ...
        if app_config.RELOAD_DB:
            self.tasks.append(asyncio.create_task(self._sync_task()))
    ...
    async def _sync_task(self):
        """Sync task at configured interval."""
        while True:
            try:
                # Perform sync
                print(
                ...
                sync_interval_seconds = app_config.SYNC_INTERVAL_MINUTES * 60
                print(f"Next sync in {app_config.SYNC_INTERVAL_MINUTES} minute(s)")
