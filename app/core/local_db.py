...
class LocalDatabaseManager:
    ...
    def _init_db(self):
        ...
        # Migrate legacy column name if DB was created with sample schema
        cursor.execute("PRAGMA table_info(persons)")
        cols = {row[1] for row in cursor.fetchall()}
        if "compId" in cols and "company_id" not in cols:
            cursor.execute("ALTER TABLE persons RENAME COLUMN compId TO company_id")
        elif "compId" in cols and "company_id" in cols:
            cursor.execute(
                "UPDATE persons SET company_id = compId WHERE company_id IS NULL"
            )
    ...
    async def save_persons(self, persons: List[Dict[str, Any]]):
        ...
        async with self._lock:
            ...
            for person in persons:
                ...
                normalized_person['company_id'] = (
                    person.get('company_id')
                    or person.get('companyId')
                    or person.get('CompanyId')
                    or person.get('compId')
                    or person.get('CompId')
                )
                ...
                values = (
                    ...
                    normalized_person.get('company_id'),
                ...
                (personId, company_id, personCode, fullname, deptName, position, jobDuties, 
    ...
    async def get_all_persons(self) -> List[Dict[str, Any]]:
        ...
        async with self._lock:
            ...
            for row in cursor.fetchall():
                ...
                # Normalize legacy key for search filter
                if person.get("company_id") is None and person.get("compId") is not None:
                    person["company_id"] = person["compId"]
