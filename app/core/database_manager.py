...
class DatabaseManager:
    ...
    def _compute_similarities_vectorized(self, face_embedding: np.ndarray, compIds: Optional[List[int]] = None) -> Tuple[np.ndarray, List[int]]:
        ...
        if compIds:
            comp_ids_set = set(compIds)
            for row_idx, person_idx in enumerate(self.person_indices):
                person = self.persons[person_idx]
                comp_id = person.get('compId')
                # Nếu comp_id bị thiếu (None) thì vẫn cho phép search, không filter out.
                if comp_id is None or comp_id in comp_ids_set:
